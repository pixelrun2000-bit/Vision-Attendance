...
function Avatar({ name, photo, size = 8, onUpload }) {
  const initials = (name || "?").split(" ").slice(0, 2).map(w => w[0]).join("").toUpperCase();
  const photoSrc = photo && photo.startsWith("/uploads") ? `${API_BASE}${photo}` : photo;
  
  const baseClass = `w-${size} h-${size} rounded-full object-cover border-2 border-white shadow-sm flex-shrink-0`;
  const placeholderClass = `w-${size} h-${size} rounded-full bg-blue-50 border-2 border-blue-100 flex flex-col items-center justify-center text-blue-500 flex-shrink-0 cursor-pointer hover:bg-blue-100 transition-colors`;

  if (photoSrc) {
    return <img src={photoSrc} alt={name} className={baseClass} />;
  }

  return (
    <div className={placeholderClass} onClick={onUpload}>
...
function PersonDetailPanel({ person, onClose }) {
  ...
  const handleUploadFace = async (userId, file) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/api/ai/enroll-user/${userId}`, {
        method: "POST",
        ...
        <div className="px-6 py-5 border-b border-gray-100 flex items-center gap-4">
          <div className="relative group">
            <Avatar 
              ...
              size={14} 
              onUpload={() => document.getElementById('panel-photo-input')?.click()}
            />
            {!person.photo_url && (
              <input 
                type="file" 
                ...
                className="hidden" 
                accept="image/*" 
                onChange={(e) => handleUploadFace(person.id, e.target.files[0])} 
              />
            )}
          </div>
                ...
                {!person.is_face_enrolled && (
                  <div className="mt-6 p-4 bg-amber-50 border border-amber-100 rounded-xl">
                    <p className="text-xs text-amber-700 font-medium mb-3">
                      <i className="fa-solid fa-triangle-exclamation mr-1" /> This person has no face data.
                    </p>
                    <label className="block w-full text-center py-2.5 bg-amber-600 text-white text-xs font-bold rounded-lg cursor-pointer hover:bg-amber-700 transition-colors">
                      {uploading ? <i className="fa-solid fa-spinner animate-spin" /> : <i className="fa-solid fa-camera mr-1" />} 
                      {uploading ? " Uploading..." : " Upload Face Photo"}
                      <input type="file" className="hidden" accept="image/*" onChange={(e) => handleUploadFace(person.id, e.target.files[0])} disabled={uploading} />
                    </label>
                  </div>
                )}
              </div>
...
function AddPersonForm({ onAdded, onClose }) {
  const [form, setForm] = useState({
    ...
    department: "", role: "employee", password: "Vision@123", photo: null,
  });
  ...
  const f = (k) => (e) => {
    ...
    setForm(p => {
      const newState = { ...p, [k]: val };
      // Auto-generate username only if user hasn't typed one yet and they are typing the name
      if (k === 'full_name_en' && !p.username) {
        newState.username = val.toLowerCase().replace(/\s+/g, '.').replace(/[^a-z0-9.]/g, '') + '.' + Math.floor(1000 + Math.random() * 9000);
      }
      return newState;
    });
  };
  ...
  const handleSave = async () => {
    ...
    const password = (form.password || "").trim() || "Vision@123";
    setSaving(true);
    setError("");
    try {
      const formData = new FormData();
      formData.append("full_name_en", form.full_name_en.trim());
      formData.append("full_name_ar", form.full_name_en.trim());
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-6 mb-5 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="col-span-2">
          <label className="text-xs font-medium text-gray-600 mb-1 block">Face Photo (For Attendance) *</label>
          <div className="flex items-center gap-4 mt-1">
            <div className="w-20 h-20 rounded-xl bg-gray-50 border-2 border-dashed border-gray-200 flex items-center justify-center overflow-hidden">
              {photoPreview ? (
                ...
                <i className="fa-solid fa-camera text-gray-300 text-xl" />
              )}
            </div>
            <div className="flex-1">
              ...
              <label 
                ...
                className="inline-flex items-center gap-2 bg-white border border-gray-200 text-gray-700 text-xs font-medium px-4 py-2 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors"
              >
                <i className="fa-solid fa-upload" /> {photoPreview ? "Change Photo" : "Upload Face Photo"}
              </label>
              <p className="text-[10px] text-gray-400 mt-1.5">Clear front-facing photo for face recognition</p>
            </div>
          </div>
        </div>
        <div className="col-span-2">
          <label className="text-xs font-medium text-gray-600 mb-1 block">Default Password *</label>
          <input className={inp} type="text" placeholder="Vision@123" value={form.password} onChange={f("password")} />
          <p className="text-[10px] text-amber-600 mt-1">
            <i className="fa-solid fa-circle-info mr-1" />
            Users can use this password to log in on the mobile app for the first time.
          </p>
        </div>
      </div>
...
export default function AddPerson() {
  ...
  const handleDeleteUser = async (id) => {
    if (!window.confirm("Are you sure you want to delete this person? This will also delete their attendance history.")) return;
    try {
