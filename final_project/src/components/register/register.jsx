...
export default function Register() {
  ...
  const validate = () => {
    const errs = {};
    if (!form.fullName.trim())     errs.fullName = "Full name (English) is required";
    if (!form.fullNameAr.trim())   errs.fullNameAr = "Full name (Arabic) is required";
      ...
      <div className="bg-white w-full max-w-2xl px-8 py-8 rounded-2xl shadow-md">
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <Field label="Full Name (English) *" id="fullName" placeholder="John Doe"
              value={form.fullName} onChange={f("fullName")} error={errors.fullName} />
            <Field label="Full Name (Arabic) *" id="fullNameAr" placeholder="جون دو"
              value={form.fullNameAr} onChange={f("fullNameAr")} error={errors.fullNameAr} />
          </div>
